...
export default function Sidebar() {
  ...
  const getOptions = (
    routes: { [path: string]: RouteOption },
    prefixPath: string
  ): React.ReactNode[] => {
    ...
    for (const optionPath in routes) {
      ...
      if (option.options) {
        content.push(
          ...getOptions(option.options, path.join(prefixPath, optionPath))
        );
      }
      ...
      nodes.push(
        <Collapsible
          ...
          defaultOpen={location.pathname.startsWith(
            path.join("/", prefixPath, optionPath)
          )}
        >
          <CollapsibleTrigger asChild>
            <Link
              ...
              to={
                option.forceExact || optionPath.startsWith("http")
                  ? optionPath
                  : option.component
                  ? path.join(prefixPath, optionPath)
                  : location.pathname
            ...
            >
              {option.title}{" "}
              {option.options ? <CaretSortIcon className="h-4 w-4" /> : <></>}
            </Link>
          </CollapsibleTrigger>
          <CollapsibleContent>{content.map((node) => node)}</CollapsibleContent>
        </Collapsible>
      );
    }
  ...
  return (
    <>
      <aside className={" top-0 sticky z-30 h-[calc(100vh-3.5rem)]"}>
        <HamburgerMenuIcon
          className={
            (open ? "hidden" : "block") +
